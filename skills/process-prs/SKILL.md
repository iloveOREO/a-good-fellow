...
The review diff MUST use the merge-base of the snapshot base and HEAD — `git diff
<base>...<head>` (three-dot), never `<base>..<head>` or a file-by-file snapshot
comparison against the base tip. A base branch that advanced after the fork makes a
two-dot diff show base-only commits as reversed deletions, fabricating "regressions"
the PR never made (this misfired on a real review once). Cross-check the changed-file
list against the GitHub PR Files API; a file that appears only in a two-dot diff is
base drift, not a PR change, and must not be reported as a finding.
